...
// Current token ID to sell (increment manually between demos)
...
app.post('/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
    ...
    if (event.type === 'checkout.session.completed') {
        ...
        // Get wallet from custom field or client_reference_id
        const buyerWallet = session.custom_fields?.[0]?.text?.value
